...
(() => {
  ...
  const state = { status: 'ALL', q: '', page: 1, pages: 1, current: null };
  const statusMeta = {
    NEW: ['Новая', 'is-new'],
  ...
  async function loadList() {
    ...
    const paginationElement = document.querySelector('[data-leads-pagination]');
    paginationElement.hidden = Number(pagination.total || 0) <= Number(pagination.limit || 20);
    document.querySelector('[data-page-label]').textContent = `Страница ${state.page} из ${state.pages}`;
    document.querySelector('[data-page-prev]').disabled = state.page <= 1;
    document.querySelector('[data-page-next]').disabled = state.page >= state.pages;
