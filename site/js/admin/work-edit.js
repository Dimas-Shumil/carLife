...
(() => {
  ...
  function getPayload() {
    return {
      ...
      durationText: field('durationText').value.trim(),
  ...
  const getImages = (kind) => (work?.images || []).filter((image) => image.kind === kind);
  ...
  function renderImageSlot(kind, container) {
    ...
    if (!images.length) {
      ...
      empty.textContent = kind === 'GALLERY' ? 'Галерея пока пустая' : 'Обложка не загружена';
  ...
  function renderImages() {
    renderImageSlot('COVER', document.querySelector('[data-images-cover]'));
  ...
  function applyWork(item) {
    ...
    for (const name of [
      'title', 'slug', 'car', 'service', 'shortDescription', 'description', 'seoTitle',
      'seoDescription', 'durationText', 'location', 'isPublished', 'showOnHome', 'sortOrder',
  ...
  async function saveWork(event) {
    ...
    try {
      const response = await admin.api(workId ? `/api/admin/works/${workId}` : '/api/admin/works', {
        method: workId ? 'PATCH' : 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(getPayload()),
      });
      admin.showToast('Работа сохранена.', 'success');
      if (!workId) return window.location.replace(`/admin/works/${response.item.id}`);
  ...
  async function uploadImages(kind, input) {
    ...
    input.disabled = true;
    try {
      const response = await admin.api(`/api/admin/works/${workId}/images`, { method: 'POST', body: data });
      applyWork(response.item);
      input.value = '';
  ...
  async function deleteImage(imageId) {
    ...
    try {
      const response = await admin.api(`/api/admin/works/${workId}/images/${imageId}`, { method: 'DELETE' });
      applyWork(response.item);
  ...
  document.querySelector('[data-upload-cover]')?.addEventListener('change', (event) => uploadImages('COVER', event.currentTarget));
  document.querySelector('[data-upload-gallery]')?.addEventListener('change', (event) => uploadImages('GALLERY', event.currentTarget));
