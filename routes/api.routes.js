...
const leadSchema = z.object({
  ...
  form_time: z.coerce.number().int().positive(),
}).strict();
...
const sendLimiter = rateLimit({
  ...
  legacyHeaders: false,
  message: { success: false, message: 'Слишком много заявок. Попробуйте чуть позже.' },
});
...
function cleanHeader(value, limit) {
...
function looksLikeSpam(data) {
  ...
  const rules = [
    ...
    /(?:seo|сео|продвижени|раскрут|таргет|лидогенерац|маркетинг)\w*/iu,
...
router.post('/send', sendLimiter, async (req, res, next) => {
  try {
    ...
    if (!parsed.success) {
      return res.status(400).json({ success: false, message: 'Проверьте заполнение формы.' });
    }

    const data = parsed.data;
    const elapsed = Date.now() - data.form_time;
    if (elapsed < MIN_FORM_TIME_MS || elapsed > 2 * 60 * 60 * 1000) {
      return res.status(400).json({ success: false, message: 'Обновите страницу и попробуйте ещё раз.' });
    }
    ...
    if (duplicate) {
      return res.json({ success: true, message: 'Заявка уже получена. Мы скоро свяжемся с вами.' });
    }
    ...
    const lead = await prisma.lead.create({
      data: {
        ...
        page: data.page,
        dedupFingerprint,
        ipAddress: cleanHeader(req.ip || req.socket?.remoteAddress, 64).replace(/^::ffff:/, ''),
        userAgent: cleanHeader(req.get('user-agent'), 500),
    ...
    return res.status(201).json({
      success: true,
      message: 'Спасибо! Заявка получена, мы скоро свяжемся с вами.',
    });
  } catch (error) {
    return next(error);
  }
